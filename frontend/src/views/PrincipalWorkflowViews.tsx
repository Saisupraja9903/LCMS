...
export function Escalations() {
  ...
  return <div className="fade-in principal-operations"><div className="page-head"><h1>Escalations</h1><p>Incoming and outgoing workflow escalations, using the existing workflow history.</p></div><div className="operations-filter"><input className="inp" value={q} onChange={e => setQ(e.target.value)} placeholder="Search request, module or initiator"/><select className="select" value={state} onChange={e => setState(e.target.value)}><option value="">All statuses</option><option value="escalated">Escalated</option></select><button className="btn btn-crimson" onClick={load}>Filter</button><button className="btn btn-out" onClick={() => { setQ(''); setState(''); setTimeout(load, 0) }}>Clear</button></div>{data.error ? <p className="principal-empty">Unable to load escalations. Please try again.</p> : <><Table title="Incoming Escalations" rows={data.incoming}/><Table title="Outgoing Escalations" rows={data.outgoing}/></>}</div>
}
